...
export default function PharmacyUsers() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await apiFetch('/admin/pharmacy-users', { navigate });
      const data = await res.json().catch(() => []);
      if (!res.ok) throw new Error(data.error || 'Unable to load pharmacy users');
  ...
  if (!isAdmin) {
    return (
      <PharmacyLayout
        ...
        subtitle="Only hospital admin can create pharmacy login accounts."
      >
        <p className="rounded-xl bg-[#f8f8f8] px-4 py-3 text-sm text-[#1f1f1f]">
          You do not have permission to manage pharmacy login accounts.
        </p>
      </PharmacyLayout>
    );
  }
  ...
  const submit = async (e) => {
    ...
    setStatus({ type: '', message: '' });
    try {
      const res = await apiFetch('/admin/add-pharmacy', {
        navigate,
        method: 'POST',
        body: JSON.stringify(form),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || 'Unable to create pharmacy user');
      setForm(emptyForm);
      setStatus({
        type: 'success',
        message: 'Pharmacy user created. They can sign in with their email and password.',
      });
      await load();
    } catch (error) {
      setStatus({ type: 'error', message: error.message });
  ...
  return (
    <PharmacyLayout
      title="Pharmacy login accounts"
      subtitle="Create pharmacy officer accounts that sign in to the pharmacy portal."
      actions={
        <button
          type="button"
          onClick={load}
    ...
    >
      ...
      <section className="mb-8 rounded-2xl border border-[#8b8b8b]/30 bg-[#ffffff] p-6 shadow-sm">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-[#1f1f1f]">Create pharmacy officer</h2>
        </div>
        <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            >
              <Plus size={16} />
              {saving ? 'Creating…' : 'Create pharmacy account'}
            </button>
          </div>
        </form>
      </section>
      ...
      <section className="rounded-2xl border border-[#8b8b8b]/30 bg-[#ffffff] p-6 shadow-sm">
        <h2 className="mb-4 text-lg font-bold text-[#1f1f1f]">Pharmacy officers</h2>
        {loading ? (
          <p className="text-sm text-[#8b8b8b]">Loading…</p>
        ) : users.length === 0 ? (
          <p className="text-sm text-[#8b8b8b]">No pharmacy login accounts yet.</p>
        ) : (
