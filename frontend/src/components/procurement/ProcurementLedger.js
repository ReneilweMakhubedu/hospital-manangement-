...
export default function ProcurementLedger() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiFetch('/procurement/ledger', { navigate });
      const data = await res.json().catch(() => []);
      if (!res.ok) throw new Error(data.error || 'Unable to load integrity ledger');
      setEvents(Array.isArray(data) ? data : data.events || data.ledger || []);
  ...
  return (
    <ProcurementLayout
      ...
      subtitle="Hash-chained procurement audit events for bid, award, contract, and payment integrity — an internal ledger, not a public blockchain network."
      actions={
        <button
          type="button"
          onClick={load}
    ...
    >
      ...
      <section className="rounded-2xl border border-[#8b8b8b]/30 bg-[#ffffff] shadow-sm">
        <div className="flex items-center gap-2 border-b border-[#8b8b8b]/30 px-5 py-4">
          ...
          <h2 className="font-bold text-[#1f1f1f]">Ledger timeline</h2>
        </div>
        ...
        {loading ? (
          ...
          <p className="px-5 py-10 text-sm text-[#8b8b8b]">No integrity ledger events yet.</p>
        ) : (
          <ol className="relative space-y-0 divide-y divide-[#8b8b8b]/25">
            {events.map((e, idx) => (
              <li key={e.id || e._id || `${e.createdAt}-${idx}`} className="flex gap-4 px-5 py-4">
                <div className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full bg-[#e41e1f]" />
                <div className="min-w-0 flex-1">
                  <div className="flex flex-wrap items-baseline justify-between gap-2">
                    <p className="font-semibold text-[#1f1f1f]">
                      {e.eventType || e.action || 'Event'}
                    </p>
                  ...
                  {(e.payloadHash || e.previousHash) && (
                    <p className="mt-2 break-all font-mono text-[10px] text-[#8b8b8b]">
                      {e.payloadHash ? `hash ${e.payloadHash}` : ''}
                      {e.previousHash ? ` · prev ${e.previousHash}` : ''}
                    </p>
                  )}
                </div>
              </li>
