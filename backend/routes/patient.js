...
router.get('/records', auth, staffOnly, (req, res) => {
  try { res.json(db.prepare(`SELECT ${patientFields} FROM users WHERE role = 'patient' ORDER BY datetime(createdAt) DESC`).all()); }
...
router.post('/records', auth, staffOnly, async (req, res) => {
  ...
  try {
    const result = db.prepare('INSERT INTO users (firstName, lastName, email, password, role, idNumber, phoneNumber, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(firstName.trim(), lastName.trim(), email.trim().toLowerCase(), await bcrypt.hash(password, 10), 'patient', idNumber.trim(), phoneNumber.trim(), address.trim());
    const patient = db.prepare(`SELECT ${patientFields} FROM users WHERE id = ?`).get(result.lastInsertRowid);
    res.status(201).json({ message: 'Patient registered successfully', patient });
  } catch (error) { res.status(400).send({ error: error.code === 'SQLITE_CONSTRAINT_UNIQUE' ? 'A patient with that email address or ID number already exists' : 'Unable to register patient' }); }
});

router.get('/profile', auth, (req, res) => {
  const patient = db.prepare(`SELECT ${patientFields} FROM users WHERE id = ? AND role = 'patient'`).get(req.user.id);
...
router.put('/profile', auth, (req, res) => {
  ...
  try {
    ...
    if (email) {
      const existingEmail = db.prepare(`SELECT id FROM users WHERE email = ? AND id != ? AND role = 'patient'`).get(email?.toLowerCase(), req.user.id);
      if (existingEmail) {
    ...
    if (idNumber) {
      const existingIdNumber = db.prepare(`SELECT id FROM users WHERE idNumber = ? AND id != ? AND role = 'patient'`).get(idNumber, req.user.id);
      if (existingIdNumber) {
    ...
    const updatedUser = db.prepare(`SELECT ${patientFields} FROM users WHERE id = ?`).get(req.user.id);
    if (!updatedUser) {
...
router.post('/documents', auth, upload.array('documents', 10), (req, res) => {
  try {
    const patient = db.prepare(`SELECT documents FROM users WHERE id = ? AND role = 'patient'`).get(req.user.id);
    ...
    db.prepare('UPDATE users SET documents = ? WHERE id = ? AND role = ?').run(JSON.stringify(mergedDocs), req.user.id, 'patient');
...
router.post('/book-appointment', auth, (req, res) => {
  const { doctorId, date, time, reason } = req.body;
  try {
    const result = db.prepare('INSERT INTO appointments (patientId, doctorId, date, time, reason) VALUES (?, ?, ?, ?, ?)').run(req.user.id, doctorId, date, time, reason);
    res.status(201).json({ message: 'Appointment booked successfully', appointment: db.prepare('SELECT id AS _id, * FROM appointments WHERE id = ?').get(result.lastInsertRowid) });
  } catch { res.status(500).send({ error: 'Server error' }); }
});

router.get('/available-slots', auth, (req, res) => {
  const booked = db.prepare('SELECT time FROM appointments WHERE doctorId = ? AND date = ?').all(req.query.doctorId, req.query.date).map((row) => row.time);
...
router.get('/appointments', auth, (req, res) => {
  const appointments = db.prepare(`SELECT a.id AS _id, a.*, d.firstName AS doctorFirstName, d.lastName AS doctorLastName FROM appointments a JOIN doctors d ON d.id = a.doctorId WHERE a.patientId = ? AND date(a.date) = date('now') ORDER BY a.time`).all(req.user.id);
  res.json(appointments);
});
router.get('/care-team', auth, (req, res) => res.json(db.prepare(`SELECT DISTINCT d.id AS _id, d.firstName, d.lastName, d.specialty FROM doctors d JOIN appointments a ON a.doctorId = d.id WHERE a.patientId = ?`).all(req.user.id)));
router.get('/prescriptions', auth, (req, res) => res.json(db.prepare(`SELECT p.id AS _id, p.*, d.firstName AS doctorFirstName, d.lastName AS doctorLastName FROM prescriptions p JOIN doctors d ON d.id = p.doctorId WHERE p.patientId = ?`).all(req.user.id)));
...
router.put('/appointments/:id/reschedule', auth, (req, res) => {
  ...
  try {
    ...
    const appointment = db.prepare('SELECT * FROM appointments WHERE id = ? AND patientId = ?').get(appointmentId, req.user.id);
    if (!appointment) {
      return res.status(404).send({ error: 'Appointment not found' });
    }

    // Check if the new slot is available (no other appointment at that time with that doctor)
    const conflict = db.prepare('SELECT id FROM appointments WHERE doctorId = ? AND date = ? AND time = ? AND id != ?').get(appointment.doctorId, date, time, appointmentId);
    if (conflict) {
      return res.status(400).send({ error: 'That time slot is not available. Please choose another.' });
    }

    // Update the appointment
    db.prepare('UPDATE appointments SET date = ?, time = ? WHERE id = ?').run(date, time, appointmentId);
