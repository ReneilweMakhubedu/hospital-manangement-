...
router.get('/', auth, staffOnly, async (req, res) => {
  try { res.json(await db.prepare(`${appointmentQuery} ORDER BY a.date ASC, a.time ASC`).all()); }
...
router.post('/', auth, staffOnly, async (req, res) => {
  ...
  const patient = await db.prepare("SELECT id FROM users WHERE id = ? AND role = 'patient'").get(patientId);
  ...
  try {
    const result = await db.prepare('INSERT INTO appointments (patientId, doctorId, date, time, reason) VALUES (?, ?, ?, ?, ?)').run(patientId, doctorId, date, time, reason.trim());
    res.status(201).json({ message: 'Appointment scheduled successfully', appointment: await db.prepare(`${appointmentQuery} WHERE a.id = ?`).get(result.lastInsertRowid) });
  } catch (error) {
    res.status(400).send({ error: error.code === '23505' ? 'That doctor already has an appointment at this time' : 'Unable to schedule appointment' });
  }
});
